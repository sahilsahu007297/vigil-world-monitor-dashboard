...
// NASA Planetary & Fireball feeds enabled

import siteConfiguration from './.figma/make/site.json' with { type: 'json' };

// Vite config — https://vitejs.dev/config/
export default defineConfig(({ mode }) => {
  const contact = loadEnv(mode, process.cwd(), 'FLIGHT_API_').FLIGHT_API_CONTACT;
  if (contact) process.env.FLIGHT_API_CONTACT = contact;
  ...
  return {
    ...
    plugins: [
      { name: 'public-aircraft-feed', configureServer(server) { server.middlewares.use(publicFeedHandler); }, configurePreviewServer(server) { server.middlewares.use(publicFeedHandler); } },
      react(),
      tailwindcss(),
    ...
    optimizeDeps: {
      include: ['react', 'react-dom', 'react-dom/client', 'd3-geo', 'topojson-client'],
      exclude: ['maplibre-gl'],
    },
    server: {
      host: '0.0.0.0',
